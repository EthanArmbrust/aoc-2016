...
//    string input = "zpqevtbw";
    ...
    while(good_keys.size() < 64){
        ...
        auto char_repeat = find_repeat(hash);
        if(char_repeat != 0){
            potential_keys.push_back({counter, char_repeat});
            for(int i = 0; i < potential_keys.size(); i++){
                ...
                if(counter - k.count_val > 1000){
                    ...
                    if(verify_repeat(hash, 5, char_repeat) && k.count_val != counter){
                        good_keys.push_back(k);
                        cout << good_keys.size() << endl;
                        potential_keys.erase(potential_keys.begin() + i);
                        break;
                    }    
                }
